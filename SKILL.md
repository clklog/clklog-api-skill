...
2. **第二步 — API Key 与部署方式**：如果未找到配置段落，使用**一次** `AskUserQuestion` 调用，包含两个问题：问题一收集 API Key（文本框输入），问题二选择部署方式（单选，2 个选项）。用户一次填完、无需多轮交互。

   **问题一 — API Key**（文本框输入，`options: []`）：
   > "请输入你的 ClkLog API Key
   >      获取步骤：1. 登录 ClkLog 后台 → 2. 进入「系统设置」→「密钥管理」→ 3. 创建并复制 API Key（格式 clk_xxxx）"
   - `options: []` — 纯文本框，无选项按钮

   **问题二 — 部署方式**（单选，2 个选项）：
   > "请选择你的 ClkLog 部署方式（直接点击选项，忽略下方输入框）"
   - `options` 只放 2 个选项，**不要多放**：
     1. **私有化部署（✅ 推荐）**：需要用户输入分析 API 和管理 API 的地址。数据更安全、性能更可控。
     2. **ClkLog 演示环境（⚠️ 仅供测试）**：分析 API 地址为 `https://demo.clklog.com/api`，管理 API 地址为 `https://demo.clklog.com/manage`。公开示例数据，不适合正式分析。
   - 选项描述中**必须清楚标明各选项的地址信息和适用场景**。
   - **注意**：该问题底部会自动附带一个"其他"自由文本输入框（工具限制），提示用户忽略即可。

   拿到回答后：
   - API Key：从问题一提取。
   - 部署方式：如果选"演示环境"，直接使用 `analytics_base_url = https://demo.clklog.com/api`、`manage_base_url = https://demo.clklog.com/manage`；如果选"私有化部署"，进入第三步让用户输入 API 地址。

3. **第三步 — 输入 ClkLog 社区版 API 地址（仅私有化部署）**：如果用户在上一步选择了"私有化部署"，使用 **一次** `AskUserQuestion` 调用同时询问分析 API 和管理 API 两个地址。**每个问题的 `options` 设为空数组 `[]`**，与 API Key 输入方式完全一致——用户直接在文本框输入，无多余选项。

   **问题一 — ClkLog 分析 API 地址**（文本框输入模式，问题文本中括号备注接口地址示例）：
   > "请输入你的 ClkLog 分析 API 地址（示例：https://yourclklogdomain.com/api）"
   - `options: []` — 纯文本框，无选项按钮

   **问题二 — ClkLog 管理 API 地址**（文本框输入模式，问题文本中括号备注接口地址示例）：
   > "请输入你的 ClkLog 管理 API 地址（示例：https://yourclklogdomain.com/manage）"
   - `options: []` — 纯文本框，无选项按钮

   **解析规则**（拿到回答后）：
   - 用户输入完整路径（含 `/api` 或 `/manage`）→ 直接使用，不追加后缀
   - 用户输入基础域名（如 `https://yourclklogdomain.com`）→ 自动补齐为 `https://yourclklogdomain.com/api`
   - 其中一个留空 → 从填写的那个推导。如只填了分析 API `https://a.com/api`，管理 API 自动推导为 `https://a.com/manage`
   - **无需追问用户补全**，直接推导后进入第四步

4. **第四步 — 获取项目列表并选择项目**：拿到 API Key 和 URL 后，立即调用管理 API `/project/getlist`（请求体含 `pageNum: 1, pageSize: 100`，建议设置较大 `pageSize` 一次性获取全部项目）获取项目列表。将项目列表以"项目名称（编码）"的形式展示给用户，使用 `AskUserQuestion` 工具让用户选择：
   - 要分析的项目（可多选）
   - 默认分析的项目（从已选项目中指定一个）

   **不要让用户手动输入项目编码**——用户通常不知道编码是什么，必须从 API 返回结果中让用户选择。

5. **持久化**：将配置和项目信息写入当前项目的 `.workbuddy/memory/MEMORY.md`，格式如下：
   ```
   ## ClkLog 社区版 API 配置
...
6. **后续使用**：每次调用 ClkLog API 时，从 `.workbuddy/memory/MEMORY.md` 读取配置值，用于：
   - 请求头 `X-API-Key`
...
- **分析 API 基础 URL**：从 `.workbuddy/memory/MEMORY.md` 读取（默认 `https://demo.clklog.com/api`）
- **管理 API 基础 URL**：从 `.workbuddy/memory/MEMORY.md` 读取（默认 `https://demo.clklog.com/manage`），仅用于获取项目列表
...
ClkLog 社区版管理 API 基础 URL 不同于分析 API（默认为 `https://demo.clklog.com/manage`），但认证方式相同（X-API-Key）。
...
1. **检查配置** — 读取 `.workbuddy/memory/MEMORY.md` 中的 ClkLog 社区版 API 配置。如果未找到，按照"配置流程"引导用户完成配置：① 一次询问 API Key + 部署方式（两个问题在一个表单，用户一次完成）→ ② 如果选了私有化部署，一次询问两个 API 地址 → ③ 调用管理 API 获取项目列表，让用户选择项目和默认项目。
...
- **管理 API URL**：项目列表接口使用 `manage_base_url`（默认 `https://demo.clklog.com/manage`），与分析 API 的 `analytics_base_url`（默认 `https://demo.clklog.com/api`）路径不同但同域同认证。
